...
import { fetchProductsThree } from '../Redux/productSlice';
...
const CarouselThree = () => {
    ...
    const { productsThree = [], statusThree, errorThree } = useSelector((state) => state.products);
    ...
    useEffect(() => {
        if (statusThree === 'idle') {
            dispatch(fetchProductsThree({ category }));
        }
    }, [dispatch, category, statusThree]);
    ...
    if (statusThree === 'loading') {
        return <div>Loading...</div>;
    }
    if (statusThree === 'failed') {
        return <div>Error: {errorThree}</div>;
    }
    ...
    return (
        <div className={`product-slider-cont ${isClickedCart ? 'clicked' : ''}`}>
            <Sliders {...settings}>
                {Array.isArray(productsThree) && productsThree.map((pro) => (
